...
public class AdmUserController /*extends BaseController*/ {
    ...
    @ResponseBody
    public CommontReturnType deleteItems(@RequestParam(name = "itemId") Integer itemId)  throws BusinessException {
        try {
    ...
    @ResponseBody
    public CommontReturnType fengHao(@RequestParam(name = "userId" ) Integer userId) throws BusinessException  {
        try {
    ...
    @ResponseBody
    public CommontReturnType jieFeng(@RequestParam(name = "userId" ) Integer userId) throws BusinessException  {
        try {
    ...
    @ResponseBody
    public CommontReturnType getErrorsLog() throws BusinessException {
        try {
            //先判断用户是否已经登录，未登录不允许通过访问
            Boolean isLogin = (Boolean) httpServletRequest.getSession().getAttribute("isLogin");
            if (isLogin == null || !isLogin) {
            ...
            return CommontReturnType.create(ErrorsLog);
        } catch (Exception e) {
            e.printStackTrace(PEM);
            String exception = SEM.toString();
            admUserService.insertErrorsLog(exception);
